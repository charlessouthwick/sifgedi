...
#Paths
gpm_dir <- "/Users/charlessouthwick/Library/CloudStorage/Box-Box/sifgedi/gpm_supp"
...
dates <- seq(ymd("2024-03-01"), ymd("2025-04-30"), by = "1 day")

#all_links <- lapply(dates, get_day_links)

# Initialize results list
all_links <- vector("list", length(dates))
names(all_links) <- as.character(dates)

for (i in seq_along(dates)) {
  date <- dates[i]
  
  cat(sprintf("[%d/%d] Fetching links for %s... ", i, length(dates), as.character(date)))
  ...
  if (is.null(links) || length(links) == 0) {
    ...
    cat(sprintf("%d link(s) found\n", length(links)))
    all_links[[i]] <- links
  }
  
  Sys.sleep(0.3)  # Be polite — avoids rate limiting / auth drops
}

file_urls <- unlist(all_links)
file_urls <- grep("\\.zip$", file_urls, value = TRUE)  # keep only the .zip files

# Save as text file — one URL per line
writeLines(file_urls, file.path(gpm_dir, "gpm_file_urls.txt"))
cat(sprintf("Saved %d URLs to gpm_file_urls.txt\n", length(file_urls)))



####
# NOW we can load these zip files and download ---------------------------------
####

#### Load the file list
file_urls <- readLines(file.path(gpm_dir, "gpm_file_urls.txt"))
...
###
# Now unzip, and select the files that end in: .total.accum. ------------------------------
###
